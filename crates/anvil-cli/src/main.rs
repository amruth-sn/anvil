...
use anvil_engine::{
    CompositionEngine, Context, FileGenerator, ServiceCategory, ServiceCombination, ServiceConfig,
    ServiceDefinition, ServicePromptType, ServiceSelection, TemplateConfig, TemplateEngine,
};
...
async fn create_project(options: CreateOptions) -> Result<()> {
        ...
        None => {
            if options.no_input {
                return Err(anyhow::anyhow!(
                    "Template must be specified when using --no-input"
                ));
            }

            println!(
                "{} No template specified, using default 'fullstack-saas'",
                "ℹ️".bright_blue()
            );
    ...
    if options.verbose {
        ...
        println!(
            "{} Dry run: {}",
            "🔍".bright_blue(),
            options.dry_run.to_string().bright_yellow()
        );
    }
    ...
    let template_config = TemplateConfig::from_file(&config_path)
        .await
    ...
    engine
        .validate_context(&context, &template_config)
    ...
    let processed_template = if !services.is_empty() {
        println!(
            "{} Using template composition with {} services...",
            "⚙️".bright_blue(),
            services.len()
        );
        ...
        if options.verbose {
            println!(
                "{} Composing template with {} services...",
                "🔧".bright_blue(),
                services.len()
            );
        }

        let composed = composition_engine
            .compose_template_with_context(&template_name, services, Some(&context))
            .await
            .map_err(|e| anyhow::anyhow!("Template composition failed: {}", e))?;

        if options.verbose {
            println!(
                "{} Composition complete, processing {} files...",
                "✅".bright_green(),
                composed.files.len()
            );
        }
    ...
    let result = generator
        .generate_files(processed_template, progress_callback)
        .await
...
async fn build_context(config: &TemplateConfig, options: &CreateOptions) -> Result<Context> {
    let mut context_builder = Context::builder().variable("project_name", options.name.clone());
            ...
            let value = match variable.default.as_ref() {
                ...
                None => {
                    ...
                    match variable.name.as_str() {
                        "project_description" => {
                            serde_yaml::Value::String("A modern SaaS application".to_string())
                        }
    ...
    // Add language if specified via CLI
    if let Some(language) = &options.language {
        context.add_variable(
            "language".to_string(),
            serde_yaml::Value::String(language.clone()),
        );
    }
    ...
    if options.verbose {
        println!(
            "{} Built context with {} variables and {} features",
            "🎯".bright_blue(),
...
fn collect_service_selections(options: &CreateOptions) -> Result<Vec<ServiceSelection>> {
    ...
    if let Some(monitoring) = &options.monitoring {
        services.push(ServiceSelection {
...
async fn collect_service_selections_interactive(
    ...
    if let Some(preset_name) = &options.preset {
        ...
        if let Some(combination) = template_config
            .service_combinations
            .iter()
            .find(|c| c.name == *preset_name)
        {
            println!(
                "{} Using preset: {}",
                "🎯".bright_magenta(),
                combination.name.bright_yellow()
            );
    ...
    for service_def in &template_config.services {
        let service_name =
            prompt_for_service_dynamic(service_def, options, &composition_engine).await?;
    ...
    if services.is_empty() {
        println!(
            "{} No services selected - creating basic template",
            "ℹ️".bright_cyan()
        );
    } else {
        println!(
...
async fn apply_service_combination(
    ...
    for service_spec in &combination.services {
        ...
        if !options.no_input {
            ...
            if let Ok(service_path) = composition_engine
                ...
                .canonicalize()
            {
                if let Ok(service_config_content) = tokio::fs::read_to_string(&service_path).await {
                    if let Ok(service_config_yaml) =
                        serde_yaml::from_str::<ServiceConfig>(&service_config_content)
                    {
                        ...
                        for prompt in &service_config_yaml.configuration_prompts {
                            ...
                            if !final_config.contains_key(&config_key) {
                                ...
                                if prompt.required {
                                    ...
                                    final_config
                                        .insert(config_key, serde_json::Value::String(value));
                                }
                            }
                        }
                    }
    ...
    println!(
        "{} Preset applied with {} services configured",
        "✅".bright_green(),
        services.len()
    );
...
async fn prompt_for_service_dynamic(
    ...
    let cli_value = match service_def.category {
        ...
        ServiceCategory::Monitoring => options.monitoring.as_ref(),
        _ => None,
    };
    ...
    let default_index = options_to_use
        .iter()
...
async fn prompt_for_service_configuration(
    ...
    let service_config_path = composition_engine
        .shared_services_path()
        .join(format!("{:?}", category).to_lowercase())
        .join(service_name)
        .join("anvil.yaml");
    ...
    for prompt in &service_config.configuration_prompts {
        let value = match prompt.prompt_type {
            ServicePromptType::Text => {
                ...
                if let Some(default_val) = &prompt.default {
                    if let Some(default_str) = default_val.as_str() {
                        text_prompt = text_prompt.with_default(default_str);
                    }
                }
            ...
            ServicePromptType::Boolean => {
                let default_bool = prompt
                    .default
                    .as_ref()
                    .and_then(|d| d.as_str().and_then(|s| s.parse::<bool>().ok()))
                    .unwrap_or(false);

                let mut confirm_prompt = Confirm::new(&prompt.prompt).with_default(default_bool);

                if let Some(description) = &prompt.description {
                    confirm_prompt = confirm_prompt.with_help_message(description);
                }

                let result = confirm_prompt
                    .prompt()
                    .map_err(|e| anyhow::anyhow!("Failed to get user input: {}", e))?;
            ...
            ServicePromptType::Select => {
                if let Some(options) = &prompt.options {
                    ...
                    return Err(anyhow::anyhow!(
                        "Select prompt '{}' is missing options",
                        prompt.name
                    ));
                }
            }
            ...
            ServicePromptType::MultiSelect => {
                if let Some(options) = &prompt.options {
                    ...
                    return Err(anyhow::anyhow!(
                        "MultiSelect prompt '{}' is missing options",
                        prompt.name
                    ));
                }
            }
...
async fn list_templates(_language: Option<String>, _format: OutputFormat) -> Result<()> {
    println!(
        "{} Template listing will be implemented in Stage 3",
        "🔮".bright_magenta()
    );
    ...
    if templates_dir.exists() {
        ...
        println!(
            "{} No templates directory found. Create 'templates/' directory with your templates.",
            "⚠️".bright_yellow()
        );
    }
...
async fn search_templates(_query: String, _limit: usize) -> Result<()> {
    println!(
        "{} Template search will be implemented in Stage 3",
        "🔮".bright_magenta()
    );
    Ok(())
}
