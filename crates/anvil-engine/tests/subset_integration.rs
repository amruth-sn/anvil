...
async fn test_template_generation_subset() {
    ...
    // Test configurations
    let test_cases = vec![
        ("rust-hello-world", "test-rust", vec![
            "Cargo.toml",
            "src/main.rs",
        ]),
        ("fullstack-saas", "test-saas", vec![
            "package.json",
            "app/page.tsx",
            "next.config.js",
        ]),
    ...
    for (template, project_name, expected_files) in test_cases {
        ...
        if output.status.success() {
            println!("    ✅ {} generated successfully in {:?}", template, duration);
            successful_tests += 1;
    ...
    assert_eq!(successful_tests, 2, "All template generation tests should pass");
    assert!(total_files_generated >= 4, "Should find most expected files");
}
...
async fn test_component_integration_subset() {
    ...
    for ui_library in &ui_libraries {
        ...
        if output.status.success() {
            ...
            println!("    ❌ Component integration failed: {}", String::from_utf8_lossy(&output.stderr));
        }
    }
    
    println!("📊 Component integration results:");
    println!("   Successful tests: {}/{}", successful_tests, ui_libraries.len());
    
    assert!(successful_tests >= 1, "At least one component integration test should pass");
}
...
async fn test_build_verification_subset() {
    ...
    if !output.status.success() {
        panic!("Failed to generate project for build verification: {}", 
    ...
    if build_output.status.success() {
        println!("    ✅ Rust project builds successfully in {:?}", build_duration);
        ...
        if run_output.status.success() {
            ...
            println!("    ⚠ Rust project compiled but runtime failed: {}", 
                     String::from_utf8_lossy(&run_output.stderr));
        }
    } else {
        println!("    ❌ Rust project build failed: {}", 
    ...
    assert!(build_output.status.success(), "Generated Rust project should build successfully");
}
...
fn test_cross_platform_compatibility() {
    ...
    assert!(anvil_binary.exists(), "Anvil binary should exist for current platform");
    ...
    assert!(output.status.success(), "Anvil should respond to --help command");
    
    let help_text = String::from_utf8_lossy(&output.stdout);
    assert!(help_text.contains("create"), "Help should mention 'create' command");
    assert!(help_text.contains("template"), "Help should mention 'template' option");
...
async fn test_performance_stress_subset() {
    ...
    println!("   Average per project: {:?}", total_duration / concurrent_count);
    
    assert_eq!(successful, concurrent_count, "All concurrent generations should succeed");
    assert!(total_duration.as_secs() < 120, "Stress test should complete within 2 minutes");
}
